...
const EXTRACT = {
  ...
  shopee: rows => { const ci = findCols(rows, { sku: ['seller sku id'], sell: ['sellable'], pend: ['pending asn inbound'], sold: ['unitssoldinlast30days'] }); const o = {}; if (ci.sku == null) return o; for (const r of rows) { const s = String(r[ci.sku] || '').trim(); if (_isSku(s)) o[s] = { sell: _num(r[ci.sell]), pend: _num(r[ci.pend]), sold: _num(r[ci.sold]) }; } return o; },
  ml: rows => { const ci = findCols(rows, { sku: ['sku'], sold: ['vendas ultimos 30 dias (un.)', 'vendas ultimos 30 dias (un)'], pend: ['entrada pendente'], aptas: ['aptas para venda'] }); const o = {}; if (ci.sku == null) return o; for (const r of rows) { const s = String(r[ci.sku] || '').trim(); if (_isSku(s) && s !== 'SKU') o[s] = { aptas: _num(r[ci.aptas]), pend: _num(r[ci.pend]), sold: _num(r[ci.sold]) }; } return o; },
